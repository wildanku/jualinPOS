    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mt-6 items-start">
        <div class="md:col-span-3 bg-white shadow rounded-lg p-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="form-control mb-3">
                    ...
                    <input type="number" value="{{ old('buy_price',$product->buy_price() ?? 0) }}" min="0" name="buy_price" class="w-full rounded @error('buy_price') border-red-600 @enderror border-gray-400">
                    @error('buy_price') <small class="text-xs text-red-600">{{ $message }}</small>@enderror
                </div>

                <div class="form-control mb-3">
                    <label for="">Sell Price</label>
                    <input type="number" value="{{ old('sell_price',$product->sell_price() ?? 0) }}" min="0" name="sell_price" class="w-full rounded @error('buy_price') border-red-600 @enderror border-gray-400">
        ...
        <div class="md:col-span-2 rounded-lg shadow bg-white p-5">
            ...
            <div class="form-control mb-3 mt-3" id="isTracked" style="@if(!old('is_tracked') && $product->is_tracked == 0) display: none @endif">
                ...
                <input type="number" value="{{ old('stock', $product->latest_stock() ?? 0) }}" name="stock" id="inputStock" @if(!old('is_tracked') && $product->is_tracked == 0) disabled @endif class="w-full rounded @error('stock') border-red-600 @enderror border-gray-400">
